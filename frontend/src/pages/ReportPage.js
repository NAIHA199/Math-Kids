...
const ReportPage = () => {
  ...
  useEffect(() => {
    const fetchReportData = async () => {
      try {
        const [summaryRes, classRes, leaderboardRes] = await Promise.all([
          fetch("http://localhost:8000/api/report/summary", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch("http://localhost:8000/api/report/class-progress", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch("http://localhost:8000/api/report/leaderboard", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
        ...
        setClassProgressData(classJson);
        setStudentLeaderboard(leaderboardJson);
        setIsLoading(false);
      } catch (error) {
        console.error("Lỗi khi fetch dữ liệu report:", error);
        setIsLoading(false);
      }
    };
                  ...
                  {classProgressData.map(cls => (
                    <div key={cls.name}>
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className="text-sm font-bold text-white">{cls.progress}%</span>
                      </div>
                      <div className="w-full bg-gray-700 rounded-full h-4">
                        <div className={`${cls.color} h-4 rounded-full`} style={{ width: `${cls.progress}%` }}></div>
                      </div>
                    </div>
                  ))}
                </div>
